...
//
//	$Id: \$
//	mips_repository_id: d_wsram_2k2way_xilinx.v, v 3.4 
//
...
module d_wsram_2k2way_xilinx(
	...
	input [2:0]	wr_data;		// Data for Tag Write
	input [0:0]	bist_to;

	/* Outputs */
	output [2:0]	rd_data;		// output from read
	...
	assign	rd_data[2:0] = ws_rd_data[2:0];
...
`endif
	...
	RAMB4_S2 d_wsram___inst1 (
		...
		.ADDR	({4'b0,line_idx}),
		.DI	({1'b0,wr_data[1]}),
		.DO	({ws_rd_data[4],ws_rd_data[1]})
		);

	RAMB4_S2 d_wsram___inst2 (
		.WE	(wr_str && wr_mask[2]),
		.EN	(en),
		.RST	(1'b0),
		.CLK	(clk),
